...
void JsonReader::SetStopsDistances(transport::Catalogue& catalogue,
    ...
    for (const auto& [stop, near_stops] : stop_to_stops_distance) {
        for (const auto& [stop_name, dist] : near_stops) {
            catalogue.SetDistance(catalogue.SearchStop(stop), catalogue.SearchStop(stop_name), dist);
        }
    }
}
...
void JsonReader::ParseBusAddRequest(const json::Dict& request_map, BusesInfoMap& buses_info) const {
    ...
    buses_info[bus_name].crug = is_roundtrip;
    auto& marshrut = buses_info[bus_name].marshrut;
    if (stops_count > 0) marshrut.reserve(is_roundtrip ? stops_count : stops_count * 2);
    for (size_t i = 0; i < bus_stops.size(); ++i) {
        marshrut.push_back(bus_stops[i].AsString());
        if (i == bus_stops.size() - 1) {
            if (!is_roundtrip) {
                buses_info[bus_name].final_stop = bus_stops[i].AsString();
                for (int j = marshrut.size() - 2; j >= 0; --j) {
                    marshrut.push_back(marshrut[j]);
                }
            }
            else {
...
void JsonReader::BusesAddProcess(transport::Catalogue& catalogue, const BusesInfoMap& buses_info) const {
    for (const auto& [name, info] : buses_info) {
        ...
        const auto& marshrut = info.marshrut;
        stop_ptrs.reserve(marshrut.size());
        for (const auto& stop : marshrut) {
            stop_ptrs.push_back(catalogue.SearchStop(stop));
        }
        catalogue.AddBus(static_cast<string>(name), stop_ptrs, info.crug);
    }
}
...
void JsonReader::SetFinals(transport::Catalogue& catalogue, const BusesInfoMap& buses_info) const {
    for (auto& [bus_name, info] : buses_info) {
        if (domain::Bus* bus = catalogue.SearchBus(bus_name)) {
            if (domain::Stop* stop = catalogue.SearchStop(info.final_stop)) {
                bus->final_stop = stop;
            }
        }
    }
