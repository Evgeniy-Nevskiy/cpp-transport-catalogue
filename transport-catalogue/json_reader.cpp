...
void JsonReader::ProcessRequests(const json::Node& stat_requests, RequestHandler& rh) const {
    ...
    for (auto& request : stat_requests.AsArray()) {
        ...
        if (type == "Bus") result.push_back(PrintBus(request_map, rh).AsDict());
...
renderer::MapRenderer JsonReader::FillRenderSettings(const json::Node& settings) const {
    json::Map request_map = settings.AsDict();
...
const json::Node JsonReader::PrintBus(const json::Map& request_map, RequestHandler& rh) const {
    json::Node result;
    ...
    if (!rh.IsBusNumber(route_number)) {
        ...
        const auto& route_info = rh.GetBus(route_number);
        result = json::Builder{}
            .StartDict()
            .Key("request_id").Value(id)
            .Key("curvature").Value(route_info->curvature)
            .Key("route_length").Value(route_info->marshrut_length)
...
const json::Node JsonReader::PrintStop(const json::Map& request_map, RequestHandler& rh) const {
    json::Node result;
...
const json::Node JsonReader::PrintMap(const json::Map& request_map, RequestHandler& rh) const {
    json::Node result;
    const int id = request_map.at("id").AsInt();
...
const json::Node JsonReader::PrintRouting(const json::Map& request_map, RequestHandler& rh) const {
    json::Node result;
