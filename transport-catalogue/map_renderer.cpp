...
namespace renderer {
    ...
    std::vector<svg::Polyline> MapRenderer::GetBusLines(const std::map<std::string_view,
        domain::Bus*>& buses_, const SphereProjector& sp) const {
        std::vector<svg::Polyline> result;
        unsigned color_num = 0;
        for (auto& [bus_name, bus_ptr] : buses_) {
            if (bus_ptr->marshrut.size() == 0) continue;
            svg::Polyline line;
            std::vector<geo::Coordinates> points;
            for (auto stop : bus_ptr->marshrut) {
    ...
    std::vector<svg::Text> MapRenderer::GetBusLabels(const std::map<std::string_view, domain::Bus*>& buses_,
        ...
        for (auto& [bus_name, bus_ptr] : buses_) {
            ...
            svg::Text text;
            text_underlayer.SetData(bus_ptr->bus_name);
            ...
            text.SetPosition(sp(bus_ptr->marshrut[0]->crds));
            text_underlayer.SetPosition(sp(bus_ptr->marshrut[0]->crds));
            result.push_back(text_underlayer);
            result.push_back(text);
            if ((!bus_ptr->crug) && (bus_ptr->final_stop) && (bus_ptr->final_stop->stop_name != bus_ptr->marshrut[0]->stop_name)) {
                svg::Text text2 = text;
                svg::Text text2_underlayer = text_underlayer;
                text2.SetPosition(sp(bus_ptr->final_stop->crds));
                text2_underlayer.SetPosition(sp(bus_ptr->final_stop->crds));
    ...
    std::vector<svg::Text> MapRenderer::GetStopLabels(const std::map<std::string_view, domain::Stop*>& marshrut,
    const SphereProjector& sp) const {
        std::vector<svg::Text> result;
        for (auto& [stop_name, stop_ptr] : marshrut) {
            svg::Text text, text_underlayer;
            text.SetPosition(sp(stop_ptr->crds));
    ...
    std::vector<svg::Circle> MapRenderer::GetStopCircles(const std::map<std::string_view, domain::Stop*>& marshrut,
        const SphereProjector& sp) const {
        std::vector<svg::Circle> result;
        for (auto& [stop_name, stop_ptr] : marshrut) {
            svg::Circle crug;
            crug.SetCenter(sp(stop_ptr->crds));
            crug.SetRadius(stop_radius_);
            crug.SetFillColor("white"s);
            result.push_back(crug);
        }
        return result;
    }

    svg::Document MapRenderer::GetSvgDocument(const std::map<std::string_view, domain::Bus*>& buses_) const {
        std::map<std::string_view, domain::Stop*> all_stops;
        std::vector<geo::Coordinates> all_coords;
        svg::Document result;
        for (const auto& [bus_name, bus_ptr] : buses_) {
            if (bus_ptr->marshrut.size() == 0) continue;
            for (const auto& stop : bus_ptr->marshrut) {
                all_stops[stop->stop_name] = stop;
                all_coords.push_back(stop->crds);
            }
        }
        SphereProjector sp(all_coords.begin(), all_coords.end(), width_, height_, padding_);
        for (const auto& line : GetBusLines(buses_, sp)) {
            result.Add(line);
        }
        for (const auto& text : GetBusLabels(buses_, sp)) {
            result.Add(text);
        }
        for (const auto& crug : GetStopCircles(all_stops, sp)) {
            result.Add(crug);
        }
    ...
    svg::Point SphereProjector::operator()(geo::Coordinates crds) const {
        return { (crds.lng - min_lon_) * zoom_coeff_ + padding_,
