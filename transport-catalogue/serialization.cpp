...
serialize::Stop Serialize(const transport::Stop* stop) {
    ...
    result.add_coordinate(stop->crds.lat);
    result.add_coordinate(stop->crds.lng);
    for (const auto& [n, d] : stop->stop_distances_) {
...
serialize::Bus Serialize(const transport::Bus* bus) {
    ...
    for (const auto& s : bus->marshrut) {
        result.add_stop(s->stop_name);
    }
    result.set_is_circle(bus->crug);
    if (bus->final_stop)
        result.set_final_stop(bus->final_stop->stop_name);
    return result;
}
...
void SetStopsDistances(transport::Catalogue& tcat, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.stop_size(); ++i) {
        ...
        transport::Stop* from = tcat.SearchStop(stop_i.name());
        for (size_t j = 0; j < stop_i.near_stop_size(); ++j) {
            tcat.SetDistance(from, tcat.SearchStop(stop_i.near_stop(j)), stop_i.distance(j));
        }
    }
}
...
void AddBusFromDB(transport::Catalogue& tcat, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.bus_size(); ++i) {
        ...
        std::vector<transport::Stop*> marshrut(bus_i.stop_size());
        for (size_t j = 0; j < marshrut.size(); ++j) {
            marshrut[j] = tcat.SearchStop(bus_i.stop(j));
        }
        tcat.AddBus(bus_i.name(), marshrut, bus_i. is_circle());
        if (!bus_i.final_stop().empty()) {
            transport::Bus* bus = tcat.SearchBus(bus_i.name());
            bus->final_stop = tcat.SearchStop(bus_i.final_stop());
        }
    }
}
