...
namespace transport {
    ...
    const graph::DirectedWeightedGraph<double>& Router::BuildGraph(const TransportCatalogue& catalogue) {
        const auto& all_stops = catalogue.SortStops();
        const auto& all_buses = catalogue.SortBuses();
        ...
        for (const auto& [stop_name, stop_info] : all_stops) {
            stop_ids[stop_info->stop_name] = vertex_id;
            stops_graph.AddEdge({
                    stop_info->stop_name,
                    0,
                    vertex_id,
        ...
        for_each(
            ...
            [&stops_graph, this, &catalogue](const auto& item) {
                ...
                const auto& stops = bus_info->marshrut;
                ...
                for (size_t i = 0; i < stops_count; ++i) {
                    for (size_t j = i + 1; j < stops_count; ++j) {
                        ...
                        stops_graph.AddEdge({ bus_info->bus_name,
                                              j - i,
